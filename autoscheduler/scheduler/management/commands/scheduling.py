...
from scheduler.utils.random_product import random_product

def _get_sections(course: Tuple[str, str], term: str) -> Dict[str, Tuple[Meeting]]:
    """ Gets all sections for each course in courses, and organizes them by section_num
    ...
    return {section_nums[section_num]: tuple(meetings)
            for section_num, meetings in groupby(meetings, key=lambda m: m.section_id)}

def _partial_schedule_valid(courses: Iterable[Tuple[str, str]],
                            meetings: Dict[str, Dict[str, Iterable[Meeting]]],
                            schedule: Tuple[int], added_i: int) -> bool:
    """ Returns whether or not a schedule containing the chosen sections is valid
        after adding the course at index added_i

    Args:
        courses: Iterable containing tuple of (subject, course number)
    ...
    """
    added_section = meetings[courses[added_i]][schedule[added_i]]
    for i, section in islice(enumerate(schedule), None, added_i):
        checking_section = meetings[courses[i]][section]
...
def _schedule_valid(courses: Iterable[Tuple[str, str]],
                    meetings: Dict[str, Dict[str, Iterable[Meeting]]],
    ...
    for i in range(1, len(schedule)):
        if not _partial_schedule_valid(courses, meetings, schedule, i):
            return False
    return True
...
def create_schedules(courses: List[Tuple[str, str]], term: str, num_schedules: int = 10):
    ...
    """
    # Create dict mapping courses to sections to meetings
    meetings = {course: _get_sections(course, term) for course in courses}
    # Get valid section numbers for each course
    valid_choices = tuple(tuple(meetings[course]) for course in courses)

    schedules = []
    # Generate random arrangements of sections and create schedules
    for schedule in random_product(*valid_choices):
        if _schedule_valid(courses, meetings, schedule):
